...
def test_case(func, args, expected, rtol=1e-12, atol=1e-12):
    ...
    actual = "<not computed>"
    try:
        # If "expected" is an exception type (or tuple of exception types), treat as "must raise"
        is_exc_type = isinstance(expected, type) and issubclass(expected, BaseException)
        is_exc_tuple = (
            isinstance(expected, tuple)
            and len(expected) > 0
            and isinstance(expected[0], type)
            and issubclass(expected[0], BaseException)
        )

        if is_exc_type or is_exc_tuple:
            try:
                func(*args)
            except expected:
                return
            raise AssertionError(f"Expected exception {expected}, but nothing was raised.")

        actual = func(*args)

        # Tuple compare (modf-style): compare elementwise, preserve signed zero
        if isinstance(expected, tuple):
            ...
            # element 0
            if isinstance(e0, float) and math.isnan(e0):
                assert isinstance(a0, float) and math.isnan(a0)
            elif isinstance(e0, float) and math.isinf(e0):
                assert math.isinf(a0)
                assert math.copysign(1.0, a0) == math.copysign(1.0, e0)
            elif a0 == 0.0 and e0 == 0.0:
                assert math.copysign(1.0, a0) == math.copysign(1.0, e0)
            else:
                assert math.isclose(a0, e0, rel_tol=rtol, abs_tol=atol)

            # element 1
            if isinstance(e1, float) and math.isnan(e1):
                assert isinstance(a1, float) and math.isnan(a1)
            elif isinstance(e1, float) and math.isinf(e1):
                assert math.isinf(a1)
                assert math.copysign(1.0, a1) == math.copysign(1.0, e1)
            elif a1 == 0.0 and e1 == 0.0:
                assert math.copysign(1.0, a1) == math.copysign(1.0, e1)
            else:
                assert math.isclose(a1, e1, rel_tol=rtol, abs_tol=atol)

            return

        # Bool cases want identity semantics (matches your original `actual is expected`)
        if isinstance(expected, bool):
            assert actual is expected
            return

        # Float-ish compare: handles nan/inf + tolerances + signed zero
        if isinstance(actual, float) or isinstance(expected, float):
            ...
            # preserve signed zero when expected encodes it (e.g. -0.0)
            if expected == 0.0 and actual == 0.0 and isinstance(expected, float):
                assert math.copysign(1.0, actual) == math.copysign(1.0, expected)
            return
...
test_case(function_list["sqrt"], (1e-12,), math.sqrt(1e-12), 5e-12, 5e-12)
...
# --- fsum
test_case(function_list["fsum"], ([0.1] * 10,), math.fsum([0.1] * 10), 0.0, 0.0)
test_case(function_list["fsum"], ([1e100, 1.0, -1e100],), math.fsum([1e100, 1.0, -1e100]), 0.0, 0.0)
...
test_case(function_list["hypot"], (1e154, 1e154), math.hypot(1e154, 1e154), 5e-12, 5e-12)
...
test_case(function_list["asin"], (1.0000000001,), ValueError)
test_case(function_list["arcsin"], (-1.0000000001,), ValueError)

test_case(function_list["acos"], (1.0000000001,), ValueError)
test_case(function_list["arccos"], (-1.0000000001,), ValueError)

test_case(function_list["sqrt"], (-1.0,), ValueError)

test_case(function_list["ln"], (0.0,), ValueError)
test_case(function_list["ln"], (-1.0,), ValueError)
test_case(function_list["log"], (-1.0,), ValueError)
test_case(function_list["lg"], (0.0,), ValueError)
test_case(function_list["lg"], (-10.0,), ValueError)
test_case(function_list["log10"], (-1.0,), ValueError)
test_case(function_list["log1p"], (-1.0,), ValueError)
test_case(function_list["log2"], (-1.0,), ValueError)

test_case(function_list["invnorm"], (0.0,), ValueError)
test_case(function_list["invnorm"], (1.0,), ValueError)

test_case(function_list["acosh"], (0.9999999999,), ValueError)

test_case(function_list["atanh"], (1.0,), ValueError)
test_case(function_list["atanh"], (-1.0,), ValueError)

test_case(function_list["comb"], (5, 6), ValueError)
test_case(function_list["comb"], (-1, 2), ValueError)
test_case(function_list["perm"], (5, 6), ValueError)
test_case(function_list["perm"], (-1, 2), ValueError)

test_case(function_list["factorial"], (-1,), ValueError)
test_case(function_list["isqrt"], (-1,), ValueError)

test_case(function_list["fmod"], (1.0, 0.0), (ValueError, ZeroDivisionError))
test_case(function_list["remainder"], (1.0, 0.0), (ValueError, ZeroDivisionError))

test_case(function_list["gamma"], (-1.0,), ValueError)
